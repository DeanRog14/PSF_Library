...
# Splits a data with indexes as headers into seperate dfs
def split_returns_cols(df, date):
    df[date] = pd.to_datetime(df[date])
    df = df.copy()
...
#### PSF COLORS ####
psf_color = ['#DB7628', '#0F3651', '#006090', '#D3EDF6', '#EDE2DA']
...
def apply_tag_modifiers(tags):
    tagset = set(tag.lower() for tag in tags)

    base_r = random.randint(100, 200)
    base_g = random.randint(0, 255)
    base_b = random.randint(0, 255)

    if "greyscale" not in tagset:
        if abs(base_r - base_g) < 20: base_g = (base_g + 40) % 256
        if abs(base_r - base_b) < 20: base_b = (base_b + 60) % 256
        if abs(base_g - base_b) < 20: base_b = (base_b + 30) % 256

    for tag in tagset:
        if tag == "pastel":
            base_r = min(255, base_r + random.randint(30, 60))
            base_g = min(255, base_g + random.randint(30, 60))
            base_b = min(255, base_b + random.randint(30, 60))

        elif tag == "muted":
            base_r = int(base_r * 0.75)
            base_g = int(base_g * 0.75)
            base_b = int(base_b * 0.75)

        elif tag == "vibrant":
            base_r = min(255, int(base_r * 1.2))
            base_g = min(255, int(base_g * 1.2))
            base_b = min(255, int(base_b * 1.2))

        elif tag == "neon":
            choices = [
                (255, random.randint(0, 50), random.randint(0, 50)),
                (random.randint(0, 50), 255, random.randint(0, 50)),
                (random.randint(0, 50), random.randint(0, 50), 255)
            ]
            base_r, base_g, base_b = random.choice(choices)

        elif tag == "earthy":
            base_r = random.randint(100, 160)
            base_g = random.randint(80, 130)
            base_b = random.randint(40, 90)

        elif tag == "greyscale":
            val = random.randint(60, 200)
            base_r = base_g = base_b = val

        elif tag == "beige":
            base_r = random.randint(220, 245)
            base_g = random.randint(200, 220)
            base_b = random.randint(170, 200)

        elif tag == "camo":
            base_r = random.randint(60, 100)
            base_g = random.randint(70, 120)
            base_b = random.randint(50, 90)
...
def generate_palette(style="vibrant rich", n=5):
    tags = style.lower().split()
    return [rgb_to_hex(apply_tag_modifiers(tags)) for _ in range(n)]
